const INTENT_PATTERNS = {
  job_search: {
    patterns: [
      ...
      /(react|angular|vue|python|java|node|golang|typescript|javascript|frontend|backend|sales|marketing|accountant|hr|design)\s+(jobs?|developer|roles?|ki\s+job|naukri)/i,
      /jobs?\s+for\s+(freshers?|seniors?|juniors?|students?|interns?)/i,
      /(apply|applying)\s+for\s+(a\s+)?(job|role|position)/i,
      /(job|naukri|naukari|kaam)\s+(chahiye|dikhao|batao|dhundo|search)/i,
      /(mujhe|hume|mere\s+liye)\s+(.*?\s+)?(job|naukri|kaam)/i,
      /(delhi|mumbai|bangalore|noida|gurgaon|hyderabad|pune|kolkata|chennai)\s+(me|mein)\s+(job|naukri)/i,
      /(ghar\s+se\s+kaam|ghar\s+baithe|wfh)/i,
    ],
    examples: [
